        ...
        if form.is_valid():
            ...
            if purchase.quantity_purchased > rice.quantity_kg:
                ...
                purchase.total_price = (Decimal(
                    purchase.quantity_purchased) * rice.price_per_kg) + purchase.delivery_cost
                purchase.save()
...
def rice_purchases_history(request):
    purchases_rice = Purchase_Rice.objects.filter(
        customer=request.user).order_by("-purchase_date")
...
def mock_customer_rice_payment(request, purchase_id):
    purchase = get_object_or_404(
        Purchase_Rice, pk=purchase_id, customer=request.user)
...
def download_receipt_for_buying_rice_for_customer(request, id):
    ...
    html_string = render_to_string(
        "customer/receipt.html", {"rice": rice, "price_per_kg": price_per_kg})
    response = HttpResponse(content_type='application/pdf')
