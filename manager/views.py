    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if rice_qty > 0 and rice_qty <= rice_stock.stock_quantity:
                ...
                rice_stock.total_price -= Decimal(rice_qty *
                                                  float(rice_stock.average_price_per_kg))
                rice_stock.save()
            else:
                messages.error(
                    request, "Invalid Quantity or insufficient stock")
                return redirect('rice_stock_report')
...
def explore_all_rice_post(request):
    if request.user.role in ['admin', 'manager', 'customer']:
        rice_posts = RicePost.objects.filter(
...
def show_my_rice_post(request):
    if request.user.role in ['manager']:
        rice_posts = RicePost.objects.filter(
            manager=request.user, is_sold=False).order_by("-created_at")
    else:
        # TODO have to add a html file for this response
...
def update_manager_profile(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            # replace with your correct URL name
...
def explore_paddy_post(request):
    ...
    top_dealer = posts.values('dealer__user__username').annotate(
        post_count=Count('id')).order_by('-post_count').first()
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if purchase.quantity_purchased > paddy.quantity:
                form.add_error('quantity_purchased',
            ...
            purchase.total_price = Decimal(
                (purchase.quantity_purchased/40.0) * float(paddy.price_per_mon)) + purchase.transport_cost
            purchase.save()

            paddy.quantity = paddy.quantity - \
                (Decimal(purchase.quantity_purchased))
            if paddy.quantity <= 0:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            purchase.total_price = (
                Decimal(purchase.quantity_purchased)*rice.price_per_kg) + purchase.delivery_cost
            purchase.save()
...
def purchase_history(request):
    purchases_paddy = Purchase_paddy.objects.filter(
        manager=request.user, status="Successful").order_by("-purchase_date")
    selling_rice = Purchase_Rice.objects.filter(
        rice__manager=request.user, status="Successful").order_by("-purchase_date")

    purchases_rice_from_others_manager = PurchaseRice.objects.filter(
        manager=request.user, status="Successful").order_by("-purchase_date")
    selling_rice_to_managers = PurchaseRice.objects.filter(
        rice__manager=request.user, status="Successful").order_by("-purchase_date")
...
def purchase_history_seen_admin(request, id):
    ...
    purchases_paddy = Purchase_paddy.objects.filter(
        manager=manager_profile.user, status="Successful").order_by("-purchase_date")
    purchases_rice = PurchaseRice.objects.filter(
        manager=manager_profile.user, status="Successful").order_by("-purchase_date")
    seling_rice = Purchase_Rice.objects.filter(
        rice__manager=manager_profile.user, status="Successful").order_by("-purchase_date")
    seling_rice_to_managers = PurchaseRice.objects.filter(
        rice__manager=manager_profile.user, status="Successful").order_by("-purchase_date")
...
def mock_paddy_payment(request, purchase_id):
    purchase = get_object_or_404(
        Purchase_paddy, pk=purchase_id, manager=request.user)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if amount == purchase.total_price:
                ...
                messages.error(
                    request, "Amount does not match the total price.")
...
def insert_phone_number(request, purchase_id):
    ...
    if request.method == 'POST':
        ...
        if phone_number == paddy.manager.managerprofile.phone_number:
            ...
            messages.error(
                request, "Wrong phone number, insert the correct number")
            return redirect("insert_phone_number", purchase_id=purchase_id)
...
def mock_rice_payment(request, rice_id):
    purchase = get_object_or_404(
        PurchaseRice, pk=rice_id, manager=request.user)
    if request.method == "POST":
...
def insert_phone_number_for_rice(request, purchase_id):
    rice = get_object_or_404(
        PurchaseRice, pk=purchase_id, manager=request.user)
...
def my_rice_order(request):
    orders = PurchaseRice.objects.filter(
        manager=request.user).order_by("-purchase_date")
...
def confirm_rice_delivery_done_by_other_manager(request, id):
    ...
    if order.status == "Delivered":
        if order.payment:
            ...
            messages.warning(
                request, "Please complete payment before confirming delivery.")
...
def incoming_order(request):
    orders = Purchase_Rice.objects.filter(
        rice__manager=request.user).order_by("-purchase_date")
    rice_orders = PurchaseRice.objects.filter(
        rice__manager=request.user).order_by("-purchase_date")
...
def accept_rice_order_from_customer(request, id):
    if request.method == "POST":
        order = get_object_or_404(
            Purchase_Rice, id=id, rice__manager=request.user)
...
def my_paddy_order(request):
    orders = Purchase_paddy.objects.filter(
        manager=request.user).order_by("-purchase_date")
...
def accept_rice_order_from_manager(request, id):
    if request.method == "POST":
        order = get_object_or_404(
            PurchaseRice, id=id, rice__manager=request.user)
        new_status = request.POST.get("new_status")
        if new_status in ["Accepted", "Cancel"] and order.status == "Pending":
            order.status = new_status
            order.save()
...
def paddy_stock_report(request):
    paddy_stocks = PaddyStockOfManager.objects.filter(
        manager=request.user).order_by('-updated_at')
    context = {
...
def process_paddy_to_rice(request, stock_id):
    stock = get_object_or_404(
        PaddyStockOfManager, id=stock_id, manager=request.user)
    ...
    if request.method == "POST":
        ...
        stock.total_price -= Decimal(process_qty *
                                     float(stock.average_price_per_kg))
        stock.save()
        ...
        rice_stock.total_price = (rice_stock.stock_quantity * float(
            rice_stock.average_price_per_kg)) + (process_qty * float(stock.average_price_per_kg))
...
def rice_stock_report(request):
    rice_stocks = RiceStock.objects.filter(
        manager=request.user).order_by('-updated_at')
    context = {
...
def download_rice_stock_report(request):
    if not WEASYPRINT_AVAILABLE:
        messages.error(
            request, "PDF generation is not available. Please install WeasyPrint dependencies.")
    ...
    html_string = render_to_string("manager/pdf_download/rice_stock_pdf.html", {
                                   'rice_stocks': rice_stocks, 'manager': request.user, 'total_rice_stock': total_rice_stock})
...
def download_paddy_stock_report(request):
    ...
    html_content = template.render(
        {"manager": manager, "paddy_stocks": paddy_stocks, "total_paddy_stock": total_paddy_stock})
...
def edit_rice_stock(request, id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.error(
                request, "There is something error, please try again")
...
def edit_paddy_stock(request, id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Update this name to your correct URL name
            return redirect('paddy_stock_report')
        else:
            messages.error(
...
def paddy_stock_update(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.error(
                request, "Paddy stock update failed, Please Try again")
...
def delete_paddy_stock(request, id):
    paddy_stock = get_object_or_404(
        PaddyStockOfManager, id=id, manager=request.user)
    if request.method == "POST":
        paddy_stock.delete()
...
def profit_loss_report_for_rice_to_manager(request):
    ...
    for selling in Owner_manager:
        ...
        # print("hello")
        selling_rice = RiceStock.objects.get(
            manager=selling.manager, rice_name=selling.rice.rice_name)
            ...
            if row.manager.managerprofile.full_name == selling_rice.manager.managerprofile.full_name:
                ...
                try:
                    stock = RiceStock.objects.get(
                        manager=request.user, rice_name=row.rice.rice_name)
                ...
                selling_price = Decimal(
                    str(row.total_price)) - Decimal(str(row.delivery_cost))
...
def download_receipt_for_buying_paddy_for_manager(request, id):
    ...
    price_per_kg = float(
        paddy.total_price-paddy.transport_cost)//float(paddy.quantity_purchased)
    ...
    html_string = render_to_string(
        "manager/pdf_download/receipt_for_buying_paddy_pdf.html", context)
    response = HttpResponse(content_type="application/pdf")
...
def download_receipt_for_buying_rice_for_manager(request, id):
    ...
    price_per_kg = float(
        rice.total_price-rice.delivery_cost)//float(rice.quantity_purchased)
    ...
    html_string = render_to_string(
        "manager/pdf_download/receipt_for_buying_rice_pdf.html", context)
    response = HttpResponse(content_type="application/pdf")
...
def download_receipt_for_selling_rice_to_customer_for_manager(request, id):
    ...
    html_string = render_to_string(
        "manager/pdf_download/receipt_for_selling_to_cuatomer_rice_pdf.html", context)
    response = HttpResponse(content_type="application/pdf")
    response["Content-Disposition"] = 'attachment; filename="receipt_for_buying_rice_pdf"'
...
def download_receipt_for_selling_rice_to_others_manager_for_manager(request, id):
    ...
    html_string = render_to_string(
        "manager/pdf_download/receipt_for_selling_rice_to_manager_pdf.html", context)
    response = HttpResponse(content_type="application/pdf")
